...
class LocalMacTransportParams(TransportParams):
    ...
    # Optional device selection (not yet implemented by helper; defaults used)
...
class _VPIOLib:
    def __init__(self, lib_path: Optional[str] = None):
        ...
        lib_path = lib_path or os.getenv("VPIO_LIB", os.path.abspath(os.path.join(os.path.dirname(__file__), "macos", "libvpio.dylib")))
    ...
    async def _pacer_metrics(self):
        ...
        while True:
            try:
                ...
                # Emit telemetry as a transport message for UI consumption
                telemetry = {
                    "avg_ms": round(avg * 1000, 2),
                    "max_ms": round(mx * 1000, 2),
                    "slow_count": slow,
                    "underflows_delta": int(delta_uf),
                    "play_ring": int(ring_play.value),
                    "cap_ring": int(ring_cap.value),
                }
                try:
                    telemetry["stage_ring"] = int(stage)
                    telemetry["stage_cap"] = int(stage_cap)
                except Exception:
                    pass

                try:
                    from pipecat.frames.frames import TransportMessageFrame
                    await self._parent._on_transport_message(
                        TransportMessageFrame(
                            message={"type": "transport-telemetry", "data": telemetry}
                        )
                    )
                except Exception:
                    pass

                # Also log to help dev experience
                try:
                    logger.info(
                        f"VPIO pacer: avg={telemetry.get('avg_ms'):.2f}ms max={telemetry.get('max_ms'):.2f}ms slow(>12ms)={slow} underflows+={delta_uf} playRing={telemetry.get('play_ring')} capRing={telemetry.get('cap_ring')} stageRing={telemetry.get('stage_ring', 0)}/{telemetry.get('stage_cap', 0)}"
                    )
                except Exception:
                    logger.info(
                        f"VPIO pacer: avg={telemetry.get('avg_ms'):.2f}ms max={telemetry.get('max_ms'):.2f}ms slow(>12ms)={slow} underflows+={delta_uf} playRing={telemetry.get('play_ring')} capRing={telemetry.get('cap_ring')}"
                    )
            except asyncio.CancelledError:
                return
...
class LocalMacTransport(BaseTransport):
    ...
    def __init__(self, params: LocalMacTransportParams, lib_path: Optional[str] = None):
        ...
        if getattr(self._params, 'input_device_name', None) or getattr(self._params, 'output_device_name', None):
            logger.warning(
                "Device selection parameters provided but not implemented; using system defaults"
            )
